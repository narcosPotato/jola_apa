...
public class HospitalList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (n == 1) {
			...
			sb.append(String.format("<a href='/apa/admin/company/entry/list.do?page=%d'>[이전 %d페이지]</a>", n - 1, blockSize));
		}
		...
		while (!(loop > blockSize || n > totalPage)) {
			...
			if (n == nowPage) {
				...
				sb.append(String.format(" <a href='/apa/admin/company/entry/list.do?page=%d'>%d</a>", n, n));
			}
		...
		if (n > totalPage) {
			...
			sb.append(String.format("<a href='/apa/admin/company/entry/list.do?page=%d'>[다음 %d페이지]</a>", n, blockSize));
		}
