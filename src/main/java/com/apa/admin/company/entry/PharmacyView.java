package com.apa.admin.company.entry;

import java.io.IOException;

import javax.servlet.RequestDispatcher;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import com.apa.model.AdminEntryDTO;
import com.apa.repository.AdminEntryDAO;

@WebServlet("/admin/company/entry/pharmacyview.do")
public class PharmacyView extends HttpServlet {
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {

		RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/views/admin/company/entry/pharmacyview.jsp");
		dispatcher.forward(req, resp);
	}
}
