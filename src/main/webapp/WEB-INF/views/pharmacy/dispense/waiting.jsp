	    ...
	    $(document).ready(function () {
	        $("#approveBtn").click(function () {
	            ...
	            if (selectedDispenseIds.length > 0) {
	                ...
	                var isApproved = confirm("선택한 항목을 승인하시겠습니까?");

	                if (isApproved) {
	                    // AJAX 요청을 통해 서버에 승인 요청 전송
	                    $.ajax({
	                        type: "POST",
	                        url: "/apa/pharmacy/dispense/approve.do",
	  ...
	  // 거절버튼
	    ...
	    $(document).ready(function () {
	        $("#rejectBtn").click(function () {
	            ...
	            if (selectedDispenseIds.length > 0) {
	                // Display confirmation dialog
	                var isRejected = confirm("선택한 항목을 거절하시겠습니까?");

	                if (isRejected) {
	                    // AJAX 요청을 통해 서버에 거절 요청 전송
	                    $.ajax({
	                        type: "POST",
	                        url: "/apa/pharmacy/dispense/decline.do",
	                        data: { dispenseIds: selectedDispenseIds },
	                        traditional: true, // 배열 파라미터 직렬화를 위해 필요
	                        success: function (response) {
	                            if (response === "Success") {
