...
<body id="page-top">
							...
							<div class="card shadow mb-4">
								...
								<div
									...
									<h6 class="m-0 font-weight-bold text-primary">제조중</h6>
									 <div id="dispense-list-btn">
	                                	<button id="list-btn" onclick="location.href='/apa/pharmacy/dispense/list.do'">전체 목록</button>
	                                </div>
								</div>
								<table id="dispensing" class="display" style="width: 100%">
							    <thead>
							        <tr>
							            <th>제조번호</th>
							            <th>병원이름</th>
							    ...
							    <tbody>
							        <c:forEach items="${dtoList}" var="dto">
							            <tr>
							                ...
							                <td><input type="checkbox" name="dispenseIds" value="${dto.dispenseSeq}"></td>							               
							            </tr>       
							        </c:forEach>        
							    </tbody>
							</table>
    ...
    $("#finishBtn").click(function () {
        var selectedDispenseIds = $("input[name='dispenseIds']:checked").map(function () {
            return $(this).val();
        }).get();

        if (selectedDispenseIds.length > 0) {
            // Display confirmation dialog
            var isFinished = confirm("선택한 항목을 제조 완료하시겠습니까?");

            if (isFinished) {
                // AJAX 요청을 통해 서버에 완료 요청 전송
                $.ajax({
                    type: "POST",
                    url: "/apa/pharmacy/dispense/dispensingfinish.do",
                    data: { dispenseIds: selectedDispenseIds },
                    traditional: true, // 배열 파라미터 직렬화를 위해 필요
                    success: function (response) {
                        if (response === "Success") {
                            alert("성공적으로 제조 완료되었습니다.");
                            location.reload(); // 페이지 다시 로드 또는 필요에 따라 UI 업데이트
                        } else if (response === "NoSelection") {
                            alert("선택된 제조번호가 없습니다.");
                        } else {
                            alert("서버 응답 오류: " + response);
                        }
                    },
                    error: function (xhr, status, error) {
                        alert("서버와의 통신 중 오류가 발생했습니다. 상태: " + status + ", 오류: " + error);
                    }
                });
            }
        } else {
